...
local function StartObserverMode( ply, mode )
	...
	if ( mode == OBS_MODE_DEATHCAM ) then
	
		ply.ViewOffsetOnDeath = ply:GetCurrentViewOffset()
		ply:SetCurrentViewOffset( vector_origin )
	
	elseif ( ply.ViewOffsetOnDeath ) then

		ply:SetPos( ply:GetPos() + ply.ViewOffsetOnDeath )
		ply.ViewOffsetOnDeath = nil
		ply:SetCurrentViewOffset( vector_origin )
